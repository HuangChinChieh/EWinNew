...
class GameRoadMap extends Component {
  constructor(props) {
    ...
    this.isLittleMap = false;

    if ('isLittleMap' in props) {
      this.isLittleMap = props.isLittleMap
    }
    //初始化路單處理API;
  ...
  render() {
    return (
      ...
      <div className={`roadMap-baccarat ${this.isLittleMap ? "is-little" : ""}`} >
        {
          this.isLittleMap ? <></> : <div className='roadMap-left'>

...
export class RoadMapAPI {
    ...
    Road3 = [];  // 小路資料
    Road4 = [];  // 曱甴路資料
    qRoadResult = null; //雜項資料
    resultString = '';
    lastResultData = null;
    mapDataCb = null;


    constructor(mapDataCb, _lang) {
        //C=CH, E=EN   
        if (_lang != null) {
            this.lang = _lang;
        }

        this.mapDataCb = mapDataCb;
    }

    initBigRoad(rawRoadConfig, bigRoadConfig) {
        // 
    ...
    setRoadMapByString(resultString) {
        ...
        if (resultString != null) {
            if (this.resultString == resultString) {
    ...
    //更新問路按鈕樣式
    updateQRoadMap(qType, imgR, imgB) {
        let qImgCfg = this.getDefaultQImg();

        switch (qType) {
            case 1:
                this.processQueryRoadImg(this.qRoadResult.road1, imgR, imgB, qImgCfg.q1r, qImgCfg.q1b);

                break;
            case 2:
                this.processQueryRoadImg(this.qRoadResult.road2, imgR, imgB, qImgCfg.q2r, qImgCfg.q2b);

                break;
            case 3:
                this.Road4processQueryRoadImg(this.qRoadResult.road3, imgR, imgB, qImgCfg.q3r, qImgCfg.q3b);
            ...
            if ((roadArray.length - 1) > arrayMax) {
                ...
                for (var i = arrayMax + 1; i < roadArray.length; i++) {
                    ...
                    if (road.dataImg != null) {
                        if ("BlinkId" in road.dataImg && road.dataImg["BlinkId"] != 0) {
                            clearInterval(road.dataImg["BlinkId"]);                            
                        }
                    ...
                    if (road.x >= xDisplay) {
                        ...
                        if (road.isQueryRoad == true) {
                            if (!("BlinkId" in road.dataImg) || road.dataImg["BlinkId"] == 0) {
                                ...
                                road.dataImg["BlinkId"] = blinkId;
                            }
                        } else {
                            if ("BlinkId" in road.dataImg && road.dataImg["BlinkId"] != 0) {
                                clearInterval(road.dataImg["BlinkId"]);
                                road.dataImg["BlinkId"] = 0;
            ...
            for (var i = 0; i < this.Road1.length; i++) {
                ...
                if (road.x >= xDisplay) {
                    ...
                    if (road.tieImg == null) {
                        ...
                        road.tieImg["RoundNumber"] = road.RoundNumber;
                    }
                    ...
                    if (road.isDragon) {
                        ...
                        if (road.dragonDiv == null) {
                            ...
                            road.dragonDiv["RoundNumber"] = road.RoundNumber;
                        }
                    } else {
                        if (road.dragonDiv != null) {
                            road.dragonDiv.style.display = "none";
                        ...
                        if (road.isDragon == true) {
                            if (road.y > 0) {
                                ...
                                if (road.isDragonStart == true) {
                                    let extendWidth = parseInt(this.bigRoadCfg.width / 2);
            ...
            for (let i = 0; i < this.rawRoad.length; i++) {
                ...
                if (road.x >= xDisplay) {
                    ...
                    if (road.dataImg == null) {
                        ...
                        road.dataImg.style.display = "none";
                        road.dataImg.style.position = "absolute";
                        road.dataImg.style.width = (this.rawRoadCfg.width - 1) + "px";
                        road.dataImg.style.height = (this.rawRoadCfg.height - 1) + "px";
                        road.dataImg["RoundNumber"] = road.RoundNumber;
                        road.dataImg.BlinkId = 0;
                    }
                    ...
                    if (road.ppImg == null) {
                        ...
                        road.ppImg.style.display = "none";
                        road.ppImg.style.position = "absolute";
                        road.ppImg.style.width = (this.rawRoadCfg.width - 1) + "px";
                        road.ppImg.style.height = (this.rawRoadCfg.height - 1) + "px";
                        road.ppImg["RoundNumber"] = road.RoundNumber;
                    }
                    ...
                    if (road.bpImg == null) {
                        ...
                        road.bpImg.style.display = "none";
                        road.bpImg.style.position = "absolute";
                        road.bpImg.style.width = (this.rawRoadCfg.width - 1) + "px";
                        road.bpImg.style.height = (this.rawRoadCfg.height - 1) + "px";
                        road.bpImg["RoundNumber"] = road.RoundNumber;
                    }
    ...
    generateVirtualDom(elementTag) {
        ...
        ret.tag = elementTag;
        ret.style = {};
        ret.getCssText = function () {
            let cssText = "";

            for (var key in ret.style) {
                cssText += key + ":" + ret.style[key] + ";";
            }

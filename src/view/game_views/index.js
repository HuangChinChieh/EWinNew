...
const GameView = (props) => {
    ...
    const handleQuery = useCallback((Q) => {
        ...
        let walletByQ = Q.UserInfo.Wallet.find(
            (x) => x.CurrencyType === props.CurrencyType
        );
    ...
    const checkIsCanBetAndCheckGameSet = () => {
        ...
        //table資訊檢查
        if (T.Status === (GameType + ".NewRound") || T.Status === (GameType + ".OpenBet") || T.Status === (GameType + ".StopBet") || T.Status === (GameType + ".GameResult") || T.Status === (GameType + ".Cancel") || T.Status === (GameType + ".Delete") || T.Status === (GameType + ".Shuffling")) {
            if (T.BaccaratType === 0 || T.BaccaratType === 1) {
                if (Q.GameSetOrder === null) {
                    if (Q.UserInfo.IsGuestAccount === false) {
                        switchCanBet(false, 12, false);
                    } else {
        ...
        } else if (T.Status === (GameType + ".Close") || T.Status === (GameType + ".NoService")) {
            switchCanBet(false, 10, false);
        } else if (T.Status === (GameType + ".AccidentPending")) {
            switchCanBet(false, 11, false);
        }
        //query資訊檢查
                    ...
                    if (Q.GameSetOrder.GameSetState === 0 || Q.GameSetOrder.GameSetState === 1) {
                        if (!Q.GameSetOrder.Cmd) {
                            if (tableInfoList[2] !== 0) {
                                ...
                                } else if (T.Status === GameType + ".StopBet") {
                                    switchCanBet(false, 8, false);
                                } else {
                                    switchCanBet(false, 9, false);
                                }
                            } else {
    ...
    const checkRealStopBet = () => {
        ...
        setIsCanBet((prev) => {
            if (prev === true) {
                tableNotify.current.notify("TableChange", {
                    tableStatus: "RealStopBet",
                });
            }
            return false;
        });
    ...
    //在不能投注的狀況下，工單指令等的額外處理 
    //0=> 預設，無指令 1=> gameSetCmd 2=> orderCmd => 3=>有投注時，是否允許取消投注 4=> 檯面數低於檯紅 5=>無法投注 6=>換桌 7=>洗牌 8=>停止投注 9=>本局已派彩
    //10=>本桌目前關閉 11=>目前事故排除中, 請稍後 12=>尚未出碼 13=>餘額不足, 請充值
    const switchCanBet = (_isCanBet, notBetAction, isFromCountDown) => {
        ...
        if (_isCanBet) {
            ...
            if (notBetAction === 1) {
                ...
                switch (cmd.toUpperCase()) {
                    ...
                    case "RequireAddChip".toUpperCase():
                        if (value != null && value !== "") {
                            ...
                            msgMaskResultControl.current.ShowMask(
                                "正在要求加彩" + reqAddChipValue + unitData.text,
                                () => { }
                            );
                        } else {
                    ...
                    case "RequireAddTips".toUpperCase():
                        // pad 要求客戶打賞小費
                        handleAddTips("addTips", {
                            tipsValue: new BigNumber(value).toNumber(),
                        });
                    ...
                    case "CancelAddChip".toUpperCase():
                        ...
                        alertMsg("小費", "已取消要求加彩" + reqAddChipValue + unitData.text,
                            () => {
                                cbRef.current.handleGameSetCmd("clearGameSetCmd", null);
                            }
                        );
                ...
                if (Q.PADAvailable === true || T.BaccaratType === 3) {
                    ...
                    if (
                        ...
                        switch (Q.GameSetOrder.GameSetState) {
                            case 2:
                                if (
                                    Q.GameSetOrder.Cmd.toUpperCase() === "Continue".toUpperCase()
                                ) {
                                    msgMaskResultControl.current.ShowMask(
                                        "正在要求繼續遊戲",
                                        () => { }
            ...
            } else if (notBetAction === 8) {
                msgMaskResultControl.current.ShowMask("停止投注", () => { });
            } else if (notBetAction === 9) {
                msgMaskResultControl.current.ShowMask("本局已派彩", () => { });
            } else if (notBetAction === 10) {
                msgMaskResultControl.current.ShowMask("本桌目前關閉", () => { });
            } else if (notBetAction === 11) {
                msgMaskResultControl.current.ShowMask("目前事故排除中, 請稍後", () => { });
            } else if (notBetAction === 12) {
                msgMaskResultControl.current.ShowMask(
                    "目前無工單, 點擊要求出碼",
                    () => {
                        requireEntry();
    ...
    const requireEntry = () => {
        ...
        if (Q !== null) {
            if (Q.UserInfo.IsGuestAccount === false) {
                alertMsg("訊息", "您尚未有傳統電投工單, 是否通知帳房進行傳統電投出碼?", () => {
                    gameClient.RequireEntry("", 0, "", (s, o) => {
                        if (s) {
                            if (o.ResultCode === 0) {
                                alertMsg("訊息", "已通知帳房人員出碼, 請稍等由帳房人員主動聯繫您", () => {
                                    history.replace("");
                                });
                            } else {
                                alertMsg("錯誤", "要求出碼錯誤: " + o.Message);
                            }
                        } else {
                            if (o === "Timeout") {
                                alertMsg("網路異常, 請重新操作");
    ...
    //   console.log("emptyOrderCount" +  "=" + emptyOrderCount);
    //   console.log("selChipData" +  "=" + selChipData);
    //   console.log("isCanBet" +  "=" + isCanBet);
    //   console.log("baccaratType" +  "=" + baccaratType);
    //   console.log("baccaratType" +  "=" + baccaratType);
    //   console.log("orderData" +  "=" + JSON.stringify(orderData));
    //   console.log("userInfo" +  "=" + JSON.stringify(userInfo));
    //   console.log("cashUnit" +  "=" + JSON.stringify(cashUnit));
    //   console.log("countdownInfo" +  "=" + JSON.stringify(countdownInfo));

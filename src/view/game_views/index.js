                  ...
                  switch (T.BaccaratType) {
                    ...
                    case 1:
                      ...
                      if (Q.SelfOrder.OrderCmd) {
                        ...
                        if (
                          countdownSecond > 0 ||
                          countdownInfo.current.tableTimeoutSecond === 0
                        ) {
                          let minBetValue = 0;
                          ...
                          if (
                            new BigNumber(Q.GameSetOrder.TotalUserChip)
                              .plus(Q.GameSetOrder.TotalRewardValue)
                              .toNumber() > minBetValue
                          ) {
                            msgMaskResultControl.current.HideMask();
      ...
      if (notBetAction === 1) {
        ...
        switch (cmd.toUpperCase()) {
          ...
          case "RequireAddChip".toUpperCase():
            if (value != null && value !== "") {
              reqAddChipValue = new BigNumber(value)
          ...
          case "CancelAddChip".toUpperCase():
            ...
            alertMsg(
              "小費",
              "已取消要求加彩" + reqAddChipValue + unitData.text,
              () => {
          ...
          case "AddChip".toUpperCase():
            ...
            alertMsg(
              "加彩成功",
              "已加彩" + reqAddChipValue + unitData.text,
              () => {
                cbRef.current.handleGameSetCmd("clearGameSetCmd", null);
              }
            );
  ...
  const checkOrderByBetLimit = (_orderData, _betLimit) => {
    ...
    if (_orderData.Banker.totalBetValue > 0) {
        ...
        if (_orderData.Banker.totalBetValue <= _betLimit.Banker.Max) {
          if (_betLimit.BetBaseBanker !== 0) {
            if (
              ...
              alertMsg(
                "提醒",
                "下注失敗, 庄注碼必須是 " + _betLimit.BetBaseBanker + " 的倍數"
              );
              return false;
            }
          }
      ...
      if (_orderData.Tie.totalBetValue >= _betLimit.Tie.Min) {
        ...
        alertMsg(
          "提醒",
          "下注失敗, 和注碼最低投注" + _betLimit.Tie.Min + getDisplayUnit().text
        );
        return false;
      }

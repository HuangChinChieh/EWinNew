var roadMapAPI = function (_lang) {
    var lang = 'C';  //C=CH, E=EN
    var rawRoadCfg = null;
    var bigRoadCfg = null;
    var road1Cfg = null;
    var road2Cfg = null;
    var road3Cfg = null;
    var blinkRoundNumber = 0; //  指定路單局數閃爍(0=不閃爍)
    var queryRoadType = 0; // 0=無問路/1=庄問路/2=閒問路
    var timerQueryRoad = null;
    var rawResultData = [];
    var rawRoad = []; // 珠盤路資料
    var Road1 = [];  // 大路資料
    var Road2 = [];  // 大眼仔資料
    var Road3 = [];  // 小路資料
    var Road4 = [];  // 曱甴路資料

    if (_lang != null)
        lang = _lang;

    this.init = function (rawRoadConfig, bigRoadConfig, road1Config, road2Config, road3Config) {
        // 
        //alert(rawRoadConfig['a']);
        rawRoadCfg = defaultRawConfig(lang);
        bigRoadCfg = defaultBigConfig();
        road1Cfg = defaultSmallConfig(1);
        road2Cfg = defaultSmallConfig(2);
        road3Cfg = defaultSmallConfig(3);

        copyConfigValue(rawRoadConfig, rawRoadCfg);
        copyConfigValue(bigRoadConfig, bigRoadCfg);
        copyConfigValue(road1Config, road1Cfg);
        copyConfigValue(road2Config, road2Cfg);
        copyConfigValue(road3Config, road3Cfg);
    };

    this.setLanguage = function (_lang) {
        if (_lang != null) {
            lang = _lang;
            initDefaultImageRaw(lang, rawRoadCfg);
            initResultData();
        }
    };

    this.setQueryRoad = function (type) {
        queryRoadType = type;
        initResultData();
    };

    this.setRoadMapByString = function (resultString) {
        if (resultString != null) {
            for (var i = 0; i < resultString.length; i++) {
                var result = resultString.substr(i, 1);

                if (rawResultData[i]) {
                    rawResultData[i].LastResult = result;
                } else {
                    rawResultData[i] = {
                        Result: "",
                        LastResult: result,
                        RoundNumber: i + 1
                    };
                }
            }

            if (resultString.length < rawResultData.length)
                rawResultData.splice(resultString.length, rawResultData.length - resultString.length);
        }

        return initResultData();
    };

    this.updateQRoadMap = function (qType, qResult, imgR, imgB) {
        var qImgCfg = getDefaultQImg();

        switch (qType) {
            case 1:
                processQueryRoadImg(qResult.road1, imgR, imgB, qImgCfg.q1r, qImgCfg.q1b);

                break;
            case 2:
                processQueryRoadImg(qResult.road2, imgR, imgB, qImgCfg.q2r, qImgCfg.q2b);

                break;
            case 3:
                processQueryRoadImg(qResult.road3, imgR, imgB, qImgCfg.q3r, qImgCfg.q3b);

                break;
        }
    };

    this.getResult = function (roundResult) {
        var retValue = {
            winner: "",
            winnerValue: 0,
            isBankerPair: false,
            isPlayerPair: false
        };

        switch (roundResult) {
            case "1":
                // 1=莊(0001)
                retValue.winner = "B";
                retValue.winnerValue = 1;
                retValue.isBankerPair = false;
                retValue.isPlayerPair = false;

                break;
            case "2":
                // 2=閒(0010)
                retValue.winner = "P";
                retValue.winnerValue = 2;
                retValue.isBankerPair = false;
                retValue.isPlayerPair = false;

                break;
            case "3":
                // 3=和(0011)
                retValue.winner = "T";
                retValue.winnerValue = 3;
                retValue.isBankerPair = false;
                retValue.isPlayerPair = false;

                break;
            case "5":
                // 5=莊+莊對(0101)
                retValue.winner = "B";
                retValue.winnerValue = 1;
                retValue.isBankerPair = true;
                retValue.isPlayerPair = false;

                break;
            case "9":
                // 9=莊+閒對(1001)
                retValue.winner = "B";
                retValue.winnerValue = 1;
                retValue.isBankerPair = false;
                retValue.isPlayerPair = true;

                break;
            case "D":
                // D=莊+莊對+閒對(1101)
                retValue.winner = "B";
                retValue.winnerValue = 1;
                retValue.isBankerPair = true;
                retValue.isPlayerPair = true;

                break;
            case "6":
                // 6=閒+莊對(0110)
                retValue.winner = "P";
                retValue.winnerValue = 2;
                retValue.isBankerPair = true;
                retValue.isPlayerPair = false;

                break;
            case "A":
                // A=閒+閒對(1010)
                retValue.winner = "P";
                retValue.winnerValue = 2;
                retValue.isBankerPair = false;
                retValue.isPlayerPair = true;

                break;
            case "E":
                // E=閒+莊對+閒對(1110)
                retValue.winner = "P";
                retValue.winnerValue = 2;
                retValue.isBankerPair = true;
                retValue.isPlayerPair = true;

                break;
            case "7":
                // 7=和+莊對(0111)
                retValue.winner = "T";
                retValue.winnerValue = 3;
                retValue.isBankerPair = true;
                retValue.isPlayerPair = false;

                break;
            case "B":
                // B=和+閒對(1011)
                retValue.winner = "T";
                retValue.winnerValue = 3;
                retValue.isBankerPair = false;
                retValue.isPlayerPair = true;

                break;
            case "F":
                // F=和+莊對+閒對(1111)
                retValue.winner = "T";
                retValue.winnerValue = 3;
                retValue.isBankerPair = true;
                retValue.isPlayerPair = true;

                break;
        }

        return retValue;
    };

    function copyConfigValue(srcCfg, dstCfg) {
        if (srcCfg != null && dstCfg != null) {
            for (var key in dstCfg) {
                if (srcCfg.hasOwnProperty(key) == true) {
                    dstCfg[key] = srcCfg[key];
                }
            }
        }
    }

    function initDefaultImageRaw(_lang, cfg) {
        if (cfg != null) {
            switch (_lang.toUpperCase()) {
                case "C":
                    cfg.pImg = "data:image/png;base64,";
                    cfg.bImg = "data:image/png;base64,";
                    cfg.tImg = "data:image/png;base64,";
                    break;
                case "E":
                    cfg.pImg = "data:image/png;base64,";
                    cfg.bImg = "data:image/png;base64,";
                    cfg.tImg = "data:image/png;base64,";
                    break;
            }

            cfg.ppImg = "data:image/png;base64,";
            cfg.bpImg = "data:image/png;base64,";
        }
    }

    function defaultRawConfig(_lang) {
        var cfg = {
            el: null,
            colMax: 20,
            x: 0,
            y: 0,
            width: 48,
            height: 48,
            enableDisplay: true,
            pImg: null,
            bImg: null,
            tImg: null,
            ppImg: null,
            bpImg: null,
            eventHandler: null
        };

        initDefaultImageRaw(_lang, cfg);

        return cfg;
    }

    function defaultBigConfig() {
        var cfg = {
            el: null,
            colMax: 20,
            x: 0,
            y: 0,
            width: 24,
            height: 24,
            enableDisplay: true,
            pImg: "data:image/png;base64,",
            bImg: "data:image/png;base64,",
            tImg: "data:image/png;base64,",
            ppImg: "data:image/png;base64,",
            bpImg: "data:image/png;base64,",
            showTieCount: false,
            eventHandler: null
        };

        return cfg;
    }

    function defaultSmallConfig(count) {
        var cfg = {
            el: null,
            colMax: 20,
            x: 0,
            y: 0,
            width: 12,
            height: 12,
            enableDisplay: true,
            rImg: "",
            bImg: "",
            extraLine: true
        };

        switch (parseInt(count)) {
            case 1:
                cfg.rImg = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAALCAYAAACprHcmAAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH3gISCQUUDEwAPAAAADxJREFUGNNjYCABMCJz/jMw/MeigBFDMZJCRlT9CA2MeBRiaGDC5SRs4kykeJBsxf9xqPlPfmgQG84kAQA3Xg8OKrOOigAAAABJRU5ErkJggg==";
                cfg.bImg = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAALCAYAAACprHcmAAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH3gISCQQfgoXo9QAAADpJREFUGNNjYCABMKJy///HooQRi2K4QmQD/iNrYMSjEEMDE24nYYozkeJBshX/x6HmPyWhQVw4kwQALV4PDvc56OYAAAAASUVORK5CYII=";
                break;
            case 2:
                cfg.rImg = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAALCAYAAACprHcmAAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH3gISCQQ7voYMJAAAADBJREFUGNNjYCABMCJz/jMw/MeigBFDMTaF6BoYCSlE1sBEipsHiWLSQ4PYcCYJAADd2QoOL/O+owAAAABJRU5ErkJggg==";
                cfg.bImg = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAALCAYAAACprHcmAAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH3gISCQQECOAhGQAAAC5JREFUGNNjYCABMKJy///HooQRi2JsClE1MBJWiNDARIqbB4lickKDuHAmCQAA09kKDhxxsQkAAAAASUVORK5CYII=";
                break;
            case 3:
                cfg.rImg = "data:image/png;base64,";
                cfg.bImg = "data:image/png;base64,";
                break;
        }

        return cfg;
    }

    function getDefaultQImg() {
        var qImg = {
            q1r: "data:image/png;base64,",
            q1b: "data:image/png;base64,",
            q2r: "data:image/png;base64,",
            q2b: "data:image/png;base64,",
            q3r: "data:image/png;base64,",
            q3b: "data:image/png;base64,"
        };

        return qImg;
    }







    function getNextRawRoad(xPos, yPos, data) {
        var road;

        // data:
        // 1: 庄
        // 2: 閒

        road = {
            x: xPos,
            y: yPos,
            data: data,
            pair: 0,
            RoundNumber: -1,
            isQueryRoad: true,
            dataImg: null,
            ppImg: null,
            bpImg: null
        };

        yPos++;
        if (yPos >= 6) {
            xPos++;
            yPos = 0;
        }

        return road;
    }

    function getNextRoad1(xPos, yPos, rTopCount, rEndCount, pointCount, lastDragon, lastData, data) {
        var road;

        // data:
        // 1: 庄
        // 2: 閒

        road = {
            x: 0,
            y: 0,
            data: 0,
            pair: 0,
            tieCount: 0,     // "和" 的計數
            colPosition: 0,  // 第幾列
            Number: 0,       // 第幾顆
            colCount: 0,     // 本列有多少路子(只紀錄在每一列的第一顆)
            RoundNumber: -1,     // 局數
            isDragon: false,
            isDragonStart: false,
            isColEnd: false,
            isQueryRoad: true,
            dataImg: null,
            bpImg: null,
            ppImg: null,
            tieImg: null,
            dragonDiv: null,
            divText: null
        };

        road.data = data;
        road.pair = 0;
        road.RoundNumber = -1;

        if (lastData != -1) {
            if (lastData != road.data) {
                // 由庄變閒或由閒變庄
                xPos++;

                rTopCount = 0;
                if (rEndCount) {
                    for (var k = 0; k < rEndCount.length; k++) {
                        if (rEndCount[k]) {
                            if (rEndCount[k] >= xPos) {
                                rTopCount = 6 - k;
                                break;
                            }
                        }
                    }
                }

                yPos = 0;
                pointCount = 1;
                lastDragon = false;

                road.x = xPos;
                road.y = yPos;
                road.colPosition = xPos;
                road.Number = (pointCount - 1);
                road.colCount = pointCount;
                road.isColEnd = true;
            } else {
                // 仍維持原本狀態
                pointCount++;
                road.colPosition = xPos;
                road.Number = (pointCount - 1);

                if ((yPos + 1) < (6 - rTopCount)) {
                    // 仍維持原位置
                    yPos++;

                    road.x = xPos;
                    road.y = yPos;
                } else {
                    var rPosition = 0;
                    // 拉龍
                    if (yPos > 0) {
                        rPosition = xPos + (pointCount - (6 - rTopCount));
                    } else {
                        rPosition = xPos;
                    }

                    if (!rEndCount[yPos]) {
                        rEndCount[yPos] = 0;
                    }

                    if (rPosition > rEndCount[yPos]) {
                        rEndCount[yPos] = rPosition;
                    }

                    if (lastDragon == false) {
                        // 龍的第一顆
                        road.isDragonStart == true;
                    }

                    lastDragon = true;

                    road.x = rPosition;
                    road.y = yPos;
                    road.isDragon = true;
                }
            }
        } else {
            pointCount = 1;
            road.colCount = pointCount;
        }

        return road;
    }

    function getNextRoad2(matchCol, lastNoMatch, xPos, yPos, rTopCount, pointCount, rEndCount, lastColor, lastRoad1, data) {
        var road; // 小路使用
        var nextColor = -1; // -1=none, 0=red, 1=blue
        var r1;
        var r2;

        // data:
        // 1: 庄
        // 2: 閒

        if (lastRoad1) {
            if (lastRoad1.data != data) {
                // 庄跳閒或閒跳庄, 比對前兩列是否等長, 等長應紀錄紅色, 不等長記藍色
                r1 = findRoad1(lastRoad1.colPosition, 0);
                r2 = findRoad1(lastRoad1.colPosition - matchCol, 0);

                lastNoMatch = false;

                if (r1) {
                    if (r2) {
                        if (r1.colCount == r2.colCount) {
                            // 等長, 記錄紅色
                            nextColor = 0;
                        } else {
                            nextColor = 1;
                        }
                    } else {
                        nextColor = -1;
                    }
                } else {
                    nextColor = -1;
                }
            } else {
                // 同色, 是否連續
                if (lastNoMatch == true) {
                    // 長排, 記錄紅色
                    nextColor = 0;
                } else {
                    // 同色, 比對是否隔壁排有路 (無路藍色, 有路紅色)
                    var r;

                    r = findRoad1(lastRoad1.colPosition - matchCol, (lastRoad1.Number + 1));
                    if (r) {
                        // 隔壁排有路, 記錄紅色
                        nextColor = 0;
                        lastNoMatch = false;
                    } else {
                        // 隔壁排無路
                        nextColor = 1;
                        lastNoMatch = true;
                    }
                }
            }

            road = {
                x: 0,
                y: 0,
                color: nextColor,
                isQueryRoad: true,
                dataImg: null
            };

            if (lastColor != -1) {
                if (lastColor == nextColor) {
                    // 原排
                    pointCount++;

                    if ((yPos + 1) < (6 - rTopCount)) {
                        // 仍維持原位置
                        yPos++;

                        road.x = xPos;
                        road.y = yPos;
                    } else {
                        var rPosition = 0;
                        // 拉龍
                        rPosition = xPos + (pointCount - (6 - rTopCount));

                        if (!rEndCount[yPos]) {
                            rEndCount[yPos] = 0;
                        }

                        if (rPosition > rEndCount[yPos]) {
                            rEndCount[yPos] = rPosition;
                        }

                        road.x = rPosition;
                        road.y = yPos;
                    }
                } else {
                    // 新排
                    xPos++;

                    rTopCount = 0;
                    if (rEndCount) {
                        for (var k = 0; k < rEndCount.length; k++) {
                            if (rEndCount[k]) {
                                if (rEndCount[k] >= xPos) {
                                    rTopCount = 6 - k;
                                    break;
                                }
                            }
                        }
                    }

                    yPos = 0;
                    pointCount = 1;

                    road.x = xPos;
                    road.y = yPos;
                }
            } else {
                yPos = 0;
                pointCount = 1;

                road.x = xPos;
                road.y = yPos;
            }
        }

        return road;
    }

    function initResultData() {
        var qRoadResult = {
            counting: null,
            road1: null,
            road2: null,
            road3: null
        };

        if (rawResultData != null) {
            var bc1;
            var pc1;
            var bc2;
            var pc2;
            var bc3;
            var pc3;

            initRawRoad();  // 處理珠盤路
            initRoad1();  // 處理大路
            qRoadResult.road1 = initRoad2(1, Road2);  // 處理大眼路
            qRoadResult.road2 = initRoad2(2, Road3);  // 處理小路
            qRoadResult.road3 = initRoad2(3, Road4);  // 處理曱甴路

            // 檢查問路系統, 避免意外出錯
            if (qRoadResult.road1.IfBanker != null)
                bc1 = qRoadResult.road1.IfBanker.color;

            if (qRoadResult.road1.IfPlayer != null)
                pc1 = qRoadResult.road1.IfPlayer.color;

            if (qRoadResult.road2.IfBanker != null)
                bc2 = qRoadResult.road1.IfBanker.color;

            if (qRoadResult.road2.IfPlayer != null)
                pc2 = qRoadResult.road1.IfPlayer.color;

            if (qRoadResult.road3.IfBanker != null)
                bc3 = qRoadResult.road1.IfBanker.color;

            if (qRoadResult.road3.IfPlayer != null)
                pc3 = qRoadResult.road1.IfPlayer.color;

            if ((bc1 == pc1) && (pc1 != -1)) {
                // 三路全不顯示
                if (qRoadResult.road1.IfBanker != null)
                    qRoadResult.road1.IfBanker.color = -1;

                if (qRoadResult.road1.IfPlayer != null)
                    qRoadResult.road1.IfPlayer.color = -1;

                if (qRoadResult.road2.IfBanker != null)
                    qRoadResult.road2.IfBanker.color = -1;

                if (qRoadResult.road2.IfPlayer != null)
                    qRoadResult.road2.IfPlayer.color = -1;

                if (qRoadResult.road3.IfBanker != null)
                    qRoadResult.road3.IfBanker.color = -1;

                if (qRoadResult.road3.IfPlayer != null)
                    qRoadResult.road3.IfPlayer.color = -1;
            } else if ((bc2 == pc2) && (pc2 != -1)) {
                // 後二路不顯示
                if (qRoadResult.road2.IfBanker != null)
                    qRoadResult.road2.IfBanker.color = -1;

                if (qRoadResult.road2.IfPlayer != null)
                    qRoadResult.road2.IfPlayer.color = -1;

                if (qRoadResult.road3.IfBanker != null)
                    qRoadResult.road3.IfBanker.color = -1;

                if (qRoadResult.road3.IfPlayer != null)
                    qRoadResult.road3.IfPlayer.color = -1;
            } else if ((bc3 == pc3) && (pc3 != -1)) {
                if (qRoadResult.road3.IfBanker != null)
                    qRoadResult.road3.IfBanker.color = -1;

                if (qRoadResult.road3.IfPlayer != null)
                    qRoadResult.road3.IfPlayer.color = -1;
            }

            qRoadResult.counting = updateCounting();  // 更新統計資料

            return qRoadResult;
        }
    }

    function processQueryRoadImg(qRoad, imgBanker, imgPlayer, redSrc, blueSrc) {
        if (qRoad) {
            if (imgBanker) {
                if (qRoad.IfBanker) {
                    if (qRoad.IfBanker.color != -1) {
                        switch (qRoad.IfBanker.color) {
                            case 0:
                                imgBanker.src = redSrc;

                                break;
                            case 1:
                                imgBanker.src = blueSrc;

                                break;
                        }
                        imgBanker.style.display = "inline";
                    } else {
                        imgBanker.style.display = "none";
                    }
                } else {
                    imgBanker.style.display = "none";
                }
            }

            if (imgPlayer) {
                if (qRoad.IfPlayer) {
                    if (qRoad.IfPlayer.color != -1) {
                        switch (qRoad.IfPlayer.color) {
                            case 0:
                                imgPlayer.src = redSrc;

                                break;
                            case 1:
                                imgPlayer.src = blueSrc;

                                break;
                        }
                        imgPlayer.style.display = "inline";
                    } else {
                        imgPlayer.style.display = "none";
                    }
                } else {
                    imgPlayer.style.display = "none";
                }
            }
        } else {
            if (imgBanker)
                imgBanker.style.display = "none";

            if (imgPlayer)
                imgPlayer.style.display = "none";
        }
    }

    // 大眼路(1)/小路(2)/曱甴路(3)
    function initRoad2(roadType, roadArray) {
        var oRoadMap = null;
        var road;
        var startPosition = -1;
        var roadPosLeft = 0;
        var roadPosTop = 0;
        var imgWidth = 0;
        var imgHeight = 0;
        var imgRed = "";
        var imgBlue = "";
        var enableDisplay = false;
        var extraLine = false;
        var matchCol = 0;
        var arrayMax = -1;
        var xMax = 0;
        var queryRoad = {
            IfPlayer: null,
            IfBanker: null
        };


        switch (roadType) {
            case 1:
                if (road1Cfg.el != null)
                    oRoadMap = road1Cfg.el;
                //else
                //    oRoadMap = document.body;

                roadPosLeft = road1Cfg.x;
                roadPosTop = road1Cfg.y;
                imgRed = road1Cfg.rImg;
                imgBlue = road1Cfg.bImg;
                matchCol = 1;
                xMax = road1Cfg.colMax;
                imgWidth = road1Cfg.width;
                imgHeight = road1Cfg.height;
                extraLine = road1Cfg.extraLine;
                enableDisplay = road1Cfg.enableDisplay;
                break;
            case 2:
                if (road2Cfg.el != null)
                    oRoadMap = road2Cfg.el;
                //else
                //    oRoadMap = document.body;

                roadPosLeft = road2Cfg.x;
                roadPosTop = road2Cfg.y;
                imgRed = road2Cfg.rImg;
                imgBlue = road2Cfg.bImg;
                matchCol = 2;
                xMax = road2Cfg.colMax;
                imgWidth = road2Cfg.width;
                imgHeight = road2Cfg.height;
                extraLine = road2Cfg.extraLine;
                enableDisplay = road2Cfg.enableDisplay;
                break;
            case 3:
                if (road3Cfg.el != null)
                    oRoadMap = road3Cfg.el;
                //else
                //    oRoadMap = document.body;

                roadPosLeft = road3Cfg.x;
                roadPosTop = road3Cfg.y;
                imgRed = road3Cfg.rImg;
                imgBlue = road3Cfg.bImg;
                matchCol = 3;
                xMax = road3Cfg.colMax;
                imgWidth = road3Cfg.width;
                imgHeight = road3Cfg.height;
                extraLine = road3Cfg.extraLine;
                enableDisplay = road3Cfg.enableDisplay;
                break;
        }

        startPosition = isRoad2Avail(matchCol);
        if (startPosition >= 0) {
            var lastNoMatch = false;
            var lastRoad1 = Road1[startPosition];
            var qR1, qR2, qColor; // 問路使用
            var xDisplay = 0;
            var xPosMax = 0;
            // 可以產生下三路結果
            var lastColor = -1;
            var j = -1;
            var xPos = 0;
            var yPos = 0;
            var pointCount = 0; // 相同顏色計算數量
            var rEndCount = []; // 拉龍拉到第幾個位置
            var rTopCount = 0; // 目前位置有幾條龍
            var nextColor = 0;  // 0=red, 1=blue

            if (Road1.length > (startPosition + 1)) {
                for (var i = (startPosition + 1); i < Road1.length; i++) {
                    lastRoad1 = Road1[i];

                    if (lastRoad1.isQueryRoad == false) {
                        if (lastRoad1.Number == 0) {
                            // 庄跳閒或閒跳庄, 比對前兩列是否等長, 等長應紀錄紅色, 不等長記藍色
                            var r1, r2;

                            r1 = findRoad1(lastRoad1.colPosition - 1, 0);
                            r2 = findRoad1(lastRoad1.colPosition - (matchCol + 1), 0);

                            lastNoMatch = false;

                            if (r1.colCount == r2.colCount) {
                                // 等長, 記錄紅色
                                nextColor = 0;
                            } else {
                                nextColor = 1;
                            }
                        } else {
                            // 同色, 是否連續
                            if (lastNoMatch == true) {
                                // 長排, 記錄紅色
                                nextColor = 0;
                            } else {
                                // 同色, 比對是否隔壁排有路 (無路藍色, 有路紅色)
                                var r;

                                r = findRoad1(lastRoad1.colPosition - matchCol, lastRoad1.Number);
                                if (r) {
                                    // 隔壁排有路, 記錄紅色
                                    nextColor = 0;
                                    lastNoMatch = false;
                                } else {
                                    // 隔壁排無路
                                    nextColor = 1;
                                    lastNoMatch = true;
                                }
                            }
                        }

                        j++;
                        if (!roadArray[j]) {
                            roadArray[j] = {
                                x: 0,
                                y: 0,
                                color: 0,
                                isQueryRoad: false,
                                dataImg: null
                            };
                        }

                        road = roadArray[j];
                        road.color = nextColor;
                        road.isQueryRoad = false;

                        if (lastColor != -1) {
                            if (lastColor == nextColor) {
                                // 原排
                                pointCount++;

                                if ((yPos + 1) < (6 - rTopCount)) {
                                    // 仍維持原位置
                                    yPos++;

                                    road.x = xPos;
                                    road.y = yPos;
                                } else {
                                    var rPosition = 0;
                                    // 拉龍
                                    rPosition = xPos + (pointCount - (6 - rTopCount));

                                    if (!rEndCount[yPos]) {
                                        rEndCount[yPos] = 0;
                                    }

                                    if (rPosition > rEndCount[yPos]) {
                                        rEndCount[yPos] = rPosition;
                                    }

                                    road.x = rPosition;
                                    road.y = yPos;
                                }
                            } else {
                                // 新排
                                xPos++;

                                rTopCount = 0;
                                for (var k = 0; k < rEndCount.length; k++) {
                                    if (rEndCount[k]) {
                                        if (rEndCount[k] >= xPos) {
                                            rTopCount = 6 - k;
                                            break;
                                        }
                                    }
                                }

                                yPos = 0;
                                pointCount = 1;

                                road.x = xPos;
                                road.y = yPos;
                            }
                        } else {
                            yPos = 0;
                            pointCount = 1;

                            road.x = xPos;
                            road.y = yPos;
                        }

                        // 尋找路單最大 x 位置
                        if (road.x > xPosMax) {
                            xPosMax = road.x;
                        }

                        lastColor = nextColor;
                    }
                }
            }

            arrayMax = j;

            // 產生問路資料
            //---------------------------------------------------------
            // 庄問路
            lastRoad1 = Road1[Road1.length - 1];
            if (lastRoad1.isQueryRoad == true)
                if (Road1.length >= 2)
                    lastRoad1 = Road1[Road1.length - 2];
                else
                    lastRoad1 = null;

            if (lastRoad1 != null) {
                queryRoad.IfBanker = getNextRoad2(matchCol, lastNoMatch, xPos, yPos, rTopCount, pointCount, rEndCount, lastColor, lastRoad1, 1);
                queryRoad.IfPlayer = getNextRoad2(matchCol, lastNoMatch, xPos, yPos, rTopCount, pointCount, rEndCount, lastColor, lastRoad1, 2);
            }

            // 0=無問路/1=庄問路/2=閒問路
            if (queryRoadType == 1) {
                arrayMax++;

                if (roadArray[arrayMax]) {
                    roadArray[arrayMax].color = queryRoad.IfBanker.color;
                    roadArray[arrayMax].x = queryRoad.IfBanker.x;
                    roadArray[arrayMax].y = queryRoad.IfBanker.y;
                } else {
                    roadArray[arrayMax] = queryRoad.IfBanker;
                }

                if (roadArray[arrayMax].x > xPosMax) {
                    xPosMax = roadArray[arrayMax].x;
                }

            } else if (queryRoadType == 2) {
                arrayMax++;

                if (roadArray[arrayMax]) {
                    roadArray[arrayMax].color = queryRoad.IfPlayer.color;
                    roadArray[arrayMax].x = queryRoad.IfPlayer.x;
                    roadArray[arrayMax].y = queryRoad.IfPlayer.y;
                } else {
                    roadArray[arrayMax] = queryRoad.IfPlayer;
                }

                if (roadArray[arrayMax].x > xPosMax) {
                    xPosMax = roadArray[arrayMax].x;
                }
            }

            // 問路產生結束
            //---------------------------------------------------------

            // 移除過多的 road (可能經過修改)
            if ((roadArray.length - 1) > arrayMax) {
                // 移除綁在畫面上的物件
                for (var i = arrayMax + 1; i < roadArray.length; i++) {
                    road = roadArray[i];

                    if (road.dataImg != null) {
                        if (road.dataImg.hasAttribute("BlinkId") == true) {
                            clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                            road.dataImg.removeAttribute("BlinkId");
                        }

                        try {
                            oRoadMap.removeChild(road.dataImg);
                        }
                        catch (e) {
                        }
                    }
                }

                roadArray.splice((arrayMax + 1), roadArray.length - (arrayMax + 1))
            }


            if ((xPosMax - (xMax - 1)) >= 0) {
                xDisplay = (xPosMax - (xMax - 1));
            } else {
                xDisplay = 0;
            }

            // 下三路呈現最多數量 x: 57
            // 處理下三路貼圖
            if ((enableDisplay == true) && (oRoadMap != null)) {
                for (var i = 0; i < roadArray.length; i++) {
                    var baseX;
                    var baseY;

                    road = roadArray[i];

                    if (road.x >= xDisplay) {
                        if (extraLine == true) {
                            baseX = roadPosLeft + ((road.x - xDisplay) * imgWidth) + parseInt((road.x - xDisplay) / 2);
                            baseY = roadPosTop + (road.y * imgHeight) + parseInt(road.y / 2);
                        } else {
                            baseX = roadPosLeft + ((road.x - xDisplay) * imgWidth);
                            baseY = roadPosTop + (road.y * imgHeight);
                        }

                        // 處理主要資料
                        if (road.dataImg == null) {
                            road.dataImg = document.createElement("img");
                            road.dataImg.style.display = "none";
                            road.dataImg.style.position = "absolute";
                            road.dataImg.style.width = (imgWidth - 1) + "px";
                            road.dataImg.style.height = (imgHeight - 1) + "px";

                            oRoadMap.appendChild(road.dataImg);
                        }

                        switch (road.color) {
                            case 0:
                                road.dataImg.src = imgRed;
                                road.dataImg.style.display = "inline";
                                break;
                            case 1:
                                road.dataImg.src = imgBlue;
                                road.dataImg.style.display = "inline";
                                break;
                            default:
                                road.dataImg.style.display = "none";
                        }

                        if (road.isQueryRoad == true) {
                            if (road.dataImg.hasAttribute("BlinkId") == false) {
                                var blinkId;

                                blinkId = setBlink(road.dataImg);
                                road.dataImg.setAttribute("BlinkId", blinkId);
                            }
                        } else {
                            if (road.dataImg.hasAttribute("BlinkId") == true) {
                                clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                                road.dataImg.removeAttribute("BlinkId");

                                clearBlink(road.dataImg);
                            }
                        }

                        // 重新設定 position
                        if (road.dataImg != null) {
                            road.dataImg.style.left = baseX + "px";
                            road.dataImg.style.top = baseY + "px";
                        }
                    } else {
                        if (road.dataImg != null) {
                            road.dataImg.style.display = "none";
                        }
                    }
                }
            }
        } else {
            if (roadArray) {
                for (var i = 0; i < roadArray.length; i++) {
                    road = roadArray[i];

                    if (road) {
                        if (road.dataImg) {
                            if (road.dataImg != null) {
                                if (road.dataImg.hasAttribute("BlinkId") == true) {
                                    clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                                    road.dataImg.removeAttribute("BlinkId");
                                }

                                try {
                                    oRoadMap.removeChild(road.dataImg);
                                }
                                catch (e) {
                                }
                            }
                        }
                    }
                }
            }

            roadArray.splice(0, roadArray.length);
        }

        return queryRoad;
    }

    // 檢查大路是否足夠產生指定小路(少一顆, 用來產生預先問路, 呼叫端需檢查)
    // 傳回值: 指定小路起始位置
    function isRoad2Avail(matchCol) {
        var startPosition = -1;

        // 必須從大路, 第N列 (matchCol), 第二口, 開始計算  // 改回傳回第一口
        // 尋找該列第一顆
        for (var i = 0; i < Road1.length; i++) {
            if (Road1[i].colPosition == matchCol && Road1[i].Number == 0) {
                startPosition = i;
                break;
            }
        }

        return startPosition;
    }

    // 尋找指定位置的大路
    function findRoad1(colPosition, Number) {
        // 傳回值: Road1 object
        var road = null;

        for (var _i = 0; _i < Road1.length; _i++) {
            if (Road1[_i].colPosition > colPosition) {
                break;
            } else if ((Road1[_i].colPosition == colPosition) && (Road1[_i].Number == Number)) {
                road = Road1[_i];
                break;
            }
        }

        return road;
    }

    // 大路
    function initRoad1() {
        var xPos = 0;
        var yPos = 0;
        var pointCount = 0; // 相同顏色計算數量
        var rEndCount = []; // 拉龍拉到第幾個位置
        var rTopCount = 0; // 目前位置有幾條龍
        var oRoadMap;;
        var j = -1;
        var arrayMax = -1;
        var started = false;
        var lastData = -1;
        var lastDragon = false;
        var roadPosLeft = bigRoadCfg.x;
        var roadPosTop = bigRoadCfg.y;
        var tieCount = 0;
        var allowNext = false;
        var road;
        var colFirstRoad;
        var xPosMax = 0;
        var xDisplay = 0;
        var queryRoad = {
            IfPlayer: null,
            IfBanker: null
        };

        function raiseEvent(o, event) {
            var r;

            r = o.getAttribute("RoundNumber");

            if (rawRoadCfg.eventHandler != null) {
                rawRoadCfg.eventHandler(event, r);
            }
        }

        if (bigRoadCfg.el != null)
            oRoadMap = bigRoadCfg.el;
        //else
        //    oRoadMap = document.body;

        // 由珠盤路轉換為大路
        if (rawRoad.length > 0) {
            for (var i = 0; i < rawRoad.length; i++) {
                if (rawRoad[i].isQueryRoad == false) {
                    switch (rawRoad[i].data) {
                        case 1:
                            allowNext = true;

                            break;
                        case 2:
                            allowNext = true;

                            break;
                        case 3:
                            tieCount++;
                            allowNext = false;

                            break;
                    }

                    if (allowNext) {
                        if (road) {
                            road.tieCount += tieCount;
                            tieCount = 0;
                        }

                        // tieCount: 如果已記入到上一顆, 此處應該為 0
                        // 不為 0 就會記入到這一顆(通常是第一顆開和造成無法計入)
                        j++;
                        if (!Road1[j]) {
                            Road1[j] = {
                                x: 0,
                                y: 0,
                                data: 0,
                                pair: 0,
                                tieCount: tieCount,     // "和" 的計數
                                colPosition: 0,  // 第幾列
                                Number: 0,       // 第幾顆
                                colCount: 0,     // 本列有多少路子(只紀錄在每一列的第一顆)
                                RoundNumber: -1,     // 局數
                                isDragon: false,
                                isDragonStart: false,
                                isColEnd: false,
                                isQueryRoad: false,
                                dataImg: null,
                                bpImg: null,
                                ppImg: null,
                                tieImg: null,
                                dragonDiv: null,
                                divText: null
                            };
                        } else {
                            // 進入這裡通常是因為修改結果
                            Road1[j].tieCount = tieCount;
                        }

                        tieCount = 0;

                        road = Road1[j];

                        road.data = rawRoad[i].data;
                        road.pair = rawRoad[i].pair;
                        road.RoundNumber = rawRoad[i].RoundNumber;
                        road.isQueryRoad = false;

                        if (lastData != -1) {
                            if (lastData != road.data) {
                                // 由庄變閒或由閒變庄
                                xPos++;

                                rTopCount = 0;
                                for (var k = 0; k < rEndCount.length; k++) {
                                    if (rEndCount[k]) {
                                        if (rEndCount[k] >= xPos) {
                                            rTopCount = 6 - k;
                                            break;
                                        }
                                    }
                                }

                                yPos = 0;
                                pointCount = 1;
                                lastDragon = false;

                                road.x = xPos;
                                road.y = yPos;
                                road.colPosition = xPos;
                                road.Number = (pointCount - 1);
                                road.colCount = pointCount;
                                road.isColEnd = true;

                                colFirstRoad = road;
                            } else {
                                // 仍維持原本狀態
                                pointCount++;
                                road.colPosition = xPos;
                                road.Number = (pointCount - 1);

                                colFirstRoad.colCount = pointCount;

                                if ((yPos + 1) < (6 - rTopCount)) {
                                    // 仍維持原位置
                                    yPos++;

                                    road.x = xPos;
                                    road.y = yPos;
                                } else {
                                    var rPosition = 0;
                                    // 拉龍
                                    if (yPos > 0) {
                                        rPosition = xPos + (pointCount - (6 - rTopCount));
                                    } else {
                                        rPosition = xPos;
                                    }

                                    if (!rEndCount[yPos]) {
                                        rEndCount[yPos] = 0;
                                    }

                                    if (rPosition > rEndCount[yPos]) {
                                        rEndCount[yPos] = rPosition;
                                    }

                                    if (lastDragon == false) {
                                        // 龍的第一顆
                                        road.isDragonStart = true;
                                    }

                                    lastDragon = true;

                                    road.x = rPosition;
                                    road.y = yPos;
                                    road.isDragon = true;
                                }
                            }
                        } else {
                            pointCount = 1;
                            road.colCount = pointCount;
                            colFirstRoad = road;
                        }

                        // 設定最後的 x 位置
                        if (road.x > xPosMax) {
                            xPosMax = road.x;
                        }

                        lastData = road.data;

                        // 設定遇到第一顆閒, 避免一開始開和
                        started = true;
                    } else {
                        // 尚未進入下一顆
                        if (road) {
                            road.tieCount += tieCount;
                            tieCount = 0;
                        } else {
                            if (j == -1) {
                                // 尚未進入第一顆大路
                                if (!Road1[0]) {
                                    Road1[0] = {
                                        x: 0,
                                        y: 0,
                                        data: 0,
                                        pair: 0,
                                        tieCount: tieCount,     // "和" 的計數
                                        colPosition: 0,  // 第幾列
                                        Number: 0,       // 第幾顆
                                        colCount: 0,     // 本列有多少路子(只紀錄在每一列的第一顆)
                                        RoundNumber: -1,     // 局數
                                        isDragon: false,
                                        isDragonStart: false,
                                        isColEnd: false,
                                        isQueryRoad: false,
                                        dataImg: null,
                                        bpImg: null,
                                        ppImg: null,
                                        tieImg: null,
                                        dragonDiv: null,
                                        divText: null
                                    };
                                } else {
                                    Road1[0].data = 0;
                                    Road1[0].tieCount = tieCount;
                                }
                            }
                        }
                    }
                }
            }

            if (j >= 0)
                arrayMax = j;
            else
                arrayMax = 0;
        } else {
            // 目前沒有路
            arrayMax = -1;
        }


        // 產生問路

        // 庄
        queryRoad.IfBanker = getNextRoad1(xPos, yPos, rTopCount, rEndCount, pointCount, lastDragon, lastData, 1)
        queryRoad.IfPlayer = getNextRoad1(xPos, yPos, rTopCount, rEndCount, pointCount, lastDragon, lastData, 2)

        // 0=無問路/1=庄問路/2=閒問路
        if (queryRoadType == 1) {
            arrayMax++;

            if (Road1[arrayMax]) {
                Road1[arrayMax].data = queryRoad.IfBanker.data;
                Road1[arrayMax].x = queryRoad.IfBanker.x;
                Road1[arrayMax].y = queryRoad.IfBanker.y;
            } else {
                Road1[arrayMax] = queryRoad.IfBanker;
            }

            if (Road1[arrayMax].x > xPosMax) {
                xPosMax = Road1[arrayMax].x;
            }

        } else if (queryRoadType == 2) {
            arrayMax++;

            if (Road1[arrayMax]) {
                Road1[arrayMax].data = queryRoad.IfPlayer.data;
                Road1[arrayMax].x = queryRoad.IfPlayer.x;
                Road1[arrayMax].y = queryRoad.IfPlayer.y;
            } else {
                Road1[arrayMax] = queryRoad.IfPlayer;
            }

            if (Road1[arrayMax].x > xPosMax) {
                xPosMax = Road1[arrayMax].x;
            }

        }
        // 問路結束


        // 移除過多的 road (可能經過修改)
        if ((Road1.length - 1) > arrayMax) {
            // 移除綁在畫面上的物件
            for (var i = arrayMax + 1; i < Road1.length; i++) {
                road = Road1[i];

                if (road.divText != null) {
                    try {
                        oRoadMap.removeChild(road.divText);
                    }
                    catch (e) {
                    }
                }

                if (road.dataImg != null) {
                    if (road.dataImg.hasAttribute("BlinkId") == true) {
                        clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                        road.dataImg.removeAttribute("BlinkId");
                    }

                    try {
                        oRoadMap.removeChild(road.dataImg);
                    }
                    catch (e) {
                    }
                }

                if (road.tieImg != null) {
                    try {
                        oRoadMap.removeChild(road.tieImg);
                    }
                    catch (e) {
                    }
                }

                if (road.ppImg != null) {
                    try {
                        oRoadMap.removeChild(road.ppImg);
                    }
                    catch (e) {
                    }
                }

                if (road.bpImg != null) {
                    try {
                        oRoadMap.removeChild(road.bpImg);
                    }
                    catch (e) {
                    }
                }

                if (road.dragonDiv != null) {
                    try {
                        oRoadMap.removeChild(road.dragonDiv);
                    }
                    catch (e) {

                    }
                }
            }

            Road1.splice((arrayMax + 1), Road1.length - (arrayMax + 1))
        }

        if ((xPosMax - (bigRoadCfg.colMax - 1)) >= 0) {
            xDisplay = (xPosMax - (bigRoadCfg.colMax - 1));
        } else {
            xDisplay = 0;
        }

        colFirstRoad = null;

        // 大路呈現最多數量 x: 32
        // 處理大路貼圖
        if ((bigRoadCfg.enableDisplay == true) && (oRoadMap != null)) {
            for (var i = 0; i < Road1.length; i++) {
                var baseX;
                var baseY;

                road = Road1[i];
                if (road.Number == 0)
                    colFirstRoad = road;

                // 只處理畫面內的
                if (road.x >= xDisplay) {
                    baseX = roadPosLeft + ((road.x - xDisplay) * bigRoadCfg.width);
                    baseY = roadPosTop + (road.y * bigRoadCfg.height);

                    // 處理主要資料
                    if (road.dataImg == null) {
                        road.dataImg = document.createElement("img");
                        road.dataImg.style.display = "none";
                        road.dataImg.style.position = "absolute";
                        road.dataImg.style.width = (bigRoadCfg.width - 1) + "px";
                        road.dataImg.style.height = (bigRoadCfg.height - 1) + "px";
                        road.dataImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 大路圖像點下去處理函式
                            if (bigRoadCfg.eventHandler != null) {
                                road.dataImg.onclick = function () { raiseEvent(this, "click"); };
                                road.dataImg.onmouseover = function () { raiseEvent(this, "mouseover"); };
                                road.dataImg.onmouseout = function () { raiseEvent(this, "mouseout"); };
                            }
                        }

                        oRoadMap.appendChild(road.dataImg);
                    }

                    if (road.divText == null) {
                        road.divText = document.createElement("div");
                        road.divText.style.display = "none";
                        road.divText.style.position = "absolute";
                        road.divText.style.color = "#008000";
                        road.divText.style.fontSize = "9px";
                        road.divText.style.width = (bigRoadCfg.width - 1) + "px";
                        road.divText.style.height = (bigRoadCfg.height - 1) + "px";
                        road.divText.style.textAlign = "center";
                        road.divText.style.lineHeight = (bigRoadCfg.height - 1) + "px";
                        road.divText.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 大路圖像點下去處理函式
                            if (bigRoadCfg.eventHandler != null) {
                                road.divText.onclick = function () { raiseEvent(this, "click"); };
                                road.divText.onmouseover = function () { raiseEvent(this, "mouseover"); };
                                road.divText.onmouseout = function () { raiseEvent(this, "mouseout"); };
                            }
                        }

                        oRoadMap.appendChild(road.divText);
                    }

                    if (road.tieImg == null) {
                        road.tieImg = document.createElement("img");
                        road.tieImg.style.display = "none";
                        road.tieImg.style.position = "absolute";
                        road.tieImg.style.width = (bigRoadCfg.width - 1) + "px";
                        road.tieImg.style.height = (bigRoadCfg.height - 1) + "px";
                        road.tieImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 大路圖像點下去處理函式
                            if (bigRoadCfg.eventHandler != null) {
                                road.tieImg.onclick = function () { raiseEvent(this, "click"); };
                                road.tieImg.onmouseover = function () { raiseEvent(this, "mouseover"); };
                                road.tieImg.onmouseout = function () { raiseEvent(this, "mouseout"); };
                            }
                        }

                        oRoadMap.appendChild(road.tieImg);
                    }

                    if (road.isDragon) {
                        // 拉龍
                        if (road.dragonDiv == null) {
                            road.dragonDiv = document.createElement("div");
                            road.dragonDiv.style.display = "none";
                            road.dragonDiv.style.position = "absolute";
                            road.dragonDiv.setAttribute("RoundNumber", road.RoundNumber);

                            if (road.isQueryRoad == false) {
                                // 大路圖像點下去處理函式
                                if (bigRoadCfg.eventHandler != null) {
                                    road.dragonDiv.onclick = function () { raiseEvent(this, "click"); };
                                    road.dragonDiv.onmouseover = function () { raiseEvent(this, "mouseover"); };
                                    road.dragonDiv.onmouseout = function () { raiseEvent(this, "mouseout"); };
                                }
                            }


                            oRoadMap.appendChild(road.dragonDiv);
                        }
                    } else {
                        if (road.dragonDiv != null) {
                            road.dragonDiv.style.display = "none";
                        }
                    }

                    switch (road.data) {
                        case 1:
                            road.dataImg.src = bigRoadCfg.bImg;
                            road.dataImg.style.display = "inline";
                            break;
                        case 2:
                            road.dataImg.src = bigRoadCfg.pImg;
                            road.dataImg.style.display = "inline";
                            break;
                        default:
                            road.dataImg.style.display = "none";
                    }

                    // 處理 "和" 數量
                    if (road.tieCount > 0) {
                        if (bigRoadCfg.showTieCount == true) {
                            if (road.tieCount > 1) {
                                road.divText.innerHTML = road.tieCount;
                                road.divText.style.display = "inline";
                            } else {
                                road.divText.innerHTML = "";
                                road.divText.style.display = "none";
                            }
                        } else {
                            road.divText.innerHTML = "";
                            road.divText.style.display = "none";
                        }

                        road.tieImg.src = bigRoadCfg.tImg;
                        road.tieImg.style.display = "inline";
                    } else {
                        road.divText.innerHTML = "";
                        road.divText.style.display = "none";
                        road.tieImg.style.display = "none";
                    }

                    // 處理對子
                    if (road.ppImg == null) {
                        road.ppImg = document.createElement("img");
                        road.ppImg.style.display = "none";
                        road.ppImg.style.position = "absolute";
                        road.ppImg.style.width = (bigRoadCfg.width - 1) + "px";
                        road.ppImg.style.height = (bigRoadCfg.height - 1) + "px";
                        road.ppImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 大路圖像點下去處理函式
                            if (bigRoadCfg.eventHandler != null) {
                                road.ppImg.onclick = function () { raiseEvent(this, "click"); };
                                road.ppImg.onmouseover = function () { raiseEvent(this, "mouseover"); };
                                road.ppImg.onmouseout = function () { raiseEvent(this, "mouseout"); };
                            }
                        }

                        oRoadMap.appendChild(road.ppImg);
                    }

                    if (road.bpImg == null) {
                        road.bpImg = document.createElement("img");
                        road.bpImg.style.display = "none";
                        road.bpImg.style.position = "absolute";
                        road.bpImg.style.width = (bigRoadCfg.width - 1) + "px";
                        road.bpImg.style.height = (bigRoadCfg.height - 1) + "px";
                        road.bpImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 大路圖像點下去處理函式
                            if (bigRoadCfg.eventHandler != null) {
                                road.bpImg.onclick = function () { raiseEvent(this, "click"); };
                                road.bpImg.onmouseover = function () { raiseEvent(this, "mouseover"); };
                                road.bpImg.onmouseout = function () { raiseEvent(this, "mouseout"); };
                            }
                        }

                        oRoadMap.appendChild(road.bpImg);
                    }

                    switch (road.pair) {
                        case 1:
                            // 庄對
                            road.bpImg.src = bigRoadCfg.bpImg;
                            road.bpImg.style.display = "inline";
                            road.ppImg.style.display = "none";
                            break;
                        case 2:
                            // 閒對
                            road.ppImg.src = bigRoadCfg.ppImg;
                            road.ppImg.style.display = "inline";
                            road.bpImg.style.display = "none";
                            break;
                        case 3:
                            // 庄閒對
                            road.bpImg.src = bigRoadCfg.bpImg;
                            road.bpImg.style.display = "inline";
                            road.ppImg.src = bigRoadCfg.ppImg;
                            road.ppImg.style.display = "inline";
                            break;
                        default:
                            road.bpImg.style.display = "none";
                            road.ppImg.style.display = "none";
                    }

                    if (road.isQueryRoad == true || road.RoundNumber == blinkRoundNumber) {
                        if (road.dataImg.hasAttribute("BlinkId") == false) {
                            var blinkId;

                            blinkId = setBlink(road.dataImg);
                            road.dataImg.setAttribute("BlinkId", blinkId);
                        }
                    } else {
                        if (road.dataImg.hasAttribute("BlinkId") == true) {
                            clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                            road.dataImg.removeAttribute("BlinkId");

                            clearBlink(road.dataImg);
                        }
                    }


                    // 重新設定 position
                    if (road.divText != null) {
                        road.divText.style.left = baseX + "px";
                        road.divText.style.top = baseY + "px";
                    }

                    if (road.dataImg != null) {
                        road.dataImg.style.left = baseX + "px";
                        road.dataImg.style.top = baseY + "px";
                    }

                    if (road.tieImg != null) {
                        road.tieImg.style.left = baseX + "px";
                        road.tieImg.style.top = baseY + "px";
                    }

                    if (road.dragonDiv != null) {
                        if (road.isDragon == true) {
                            if (road.y > 0) {
                                road.dragonDiv.style.top = parseInt((baseY + (bigRoadCfg.height / 2)) - 1) + "px";
                                road.dragonDiv.style.height = "1px";
                                road.dragonDiv.style.lineHeight = "1px";
                                road.dragonDiv.style.display = "inline";
                                road.dragonDiv.innerHTML = "";

                                switch (road.data) {
                                    case 1:  //庄
                                        road.dragonDiv.style.backgroundColor = "#ff0000";
                                        break;
                                    case 2:  //閒
                                        road.dragonDiv.style.backgroundColor = "#0000ff";
                                        break;
                                }

                                if (road.isDragonStart == true) {
                                    var extenWidth = parseInt(bigRoadCfg.width / 2);

                                    if (road.Number == (colFirstRoad.colCount - 1)) {
                                        // dragon start = dragon end
                                        road.dragonDiv.style.left = parseInt(baseX - extenWidth) + "px";
                                        road.dragonDiv.style.width = bigRoadCfg.width + "px";
                                    } else {
                                        // dragon start
                                        road.dragonDiv.style.left = parseInt(baseX - extenWidth) + "px";
                                        road.dragonDiv.style.width = (bigRoadCfg.width + extenWidth) + "px";
                                    }
                                } else if (road.Number == (colFirstRoad.colCount - 1)) {
                                    // dragon end
                                    road.dragonDiv.style.left = baseX + "px";
                                    road.dragonDiv.style.width = parseInt(bigRoadCfg.width / 2) + "px";
                                } else {
                                    // dragon body
                                    road.dragonDiv.style.left = baseX + "px";
                                    road.dragonDiv.style.width = bigRoadCfg.width + "px";
                                }
                            } else {
                                // 龍不能是第一顆
                                // 只能呈現文字
                                if (colFirstRoad.colCount > 1) {
                                    road.dragonDiv.style.left = baseX + "px";
                                    road.dragonDiv.style.top = baseY + "px";
                                    road.dragonDiv.style.width = bigRoadCfg.width + "px";
                                    road.dragonDiv.style.height = bigRoadCfg.height + "px";
                                    road.dragonDiv.style.lineHeight = bigRoadCfg.height + "px";
                                    road.dragonDiv.style.textAlign = "center";

                                    switch (road.data) {
                                        case 1:  //庄
                                            road.dragonDiv.style.backgroundColor = "";
                                            road.dragonDiv.style.color = "#ff0000";
                                            break;
                                        case 2:  //閒
                                            road.dragonDiv.style.backgroundColor = "";
                                            road.dragonDiv.style.color = "#0000ff";
                                            break;
                                    }

                                    road.dragonDiv.innerHTML = colFirstRoad.colCount;
                                    road.dragonDiv.style.display = "inline";
                                } else {
                                    road.dragonDiv.style.display = "none";
                                }
                            }
                        }
                    }

                    if (road.ppImg != null) {
                        road.ppImg.style.left = baseX + "px";
                        road.ppImg.style.top = baseY + "px";
                    }

                    if (road.bpImg != null) {
                        road.bpImg.style.left = baseX + "px";
                        road.bpImg.style.top = baseY + "px";
                    }
                } else {
                    if (road.divText != null) {
                        road.divText.style.display = "none";
                    }

                    if (road.dataImg != null) {
                        road.dataImg.style.display = "none";
                    }

                    if (road.dragonDiv != null) {
                        road.dragonDiv.style.display = "none";
                    }

                    if (road.ppImg != null) {
                        road.ppImg.style.display = "none";
                    }

                    if (road.bpImg != null) {
                        road.bpImg.style.display = "none";
                    }

                    if (road.tieImg != null) {
                        road.tieImg.style.display = "none";
                    }
                }
            }
        }
    }

    // 珠盤路
    function initRawRoad() {
        var xPos = 0;
        var yPos = 0;
        var oRoadMap = null;
        var roadPosLeft = rawRoadCfg.x;
        var roadPosTop = rawRoadCfg.y;
        var xPosMax = 0;
        var xDisplay = 0;
        var arrayMax = -1;
        var queryRoad = {
            IfPlayer: null,
            IfBanker: null
        };


        function raiseEvent(o, event) {
            var r;

            r = o.getAttribute("RoundNumber");

            if (rawRoadCfg.eventHandler != null) {
                rawRoadCfg.eventHandler(event, r);
            }
        }

        if (rawRoadCfg.el != null) {
            oRoadMap = rawRoadCfg.el;
        } //else {
        //  oRoadMap = document.body;
        //}

        // rawResultData[x].LastResult:
        // 1=莊(0001)/2=閒(0010)/3=和(0011)/5=莊+莊對(0101)/9=莊+閒對(1001)/D=莊+莊對+閒對(1101)/6=閒+莊對(0110)/A=閒+閒對(1010)/E=閒+莊對+閒對(1110)/7=和+莊對(0111)/B=和+閒對(1011)/F=和+莊對+閒對(1111)
        for (var i = 0; i < rawResultData.length; i++) {
            var rData = 0;  //1=庄/2=閒/3=和
            var rPair = 0;  //0=無對/1=庄對/2=閒對/3=庄閒對
            var valueChange = false;

            if (rawResultData[i].Result != rawResultData[i].LastResult) {
                rawResultData[i].Result = rawResultData[i].LastResult
                valueChange = true;
            }

            switch (rawResultData[i].LastResult) {
                case "1":
                    // 1=莊(0001)
                    rData = 1;
                    rPair = 0;
                    break;
                case "2":
                    // 2=閒(0010)
                    rData = 2;
                    rPair = 0;
                    break;
                case "3":
                    // 3=和(0011)
                    rData = 3;
                    rPair = 0;
                    break;
                case "5":
                    // 5=莊+莊對(0101)
                    rData = 1;
                    rPair = 1;
                    break;
                case "9":
                    // 9=莊+閒對(1001)
                    rData = 1;
                    rPair = 2;
                    break;
                case "D":
                    // D=莊+莊對+閒對(1101)
                    rData = 1;
                    rPair = 3;
                    break;
                case "6":
                    // 6=閒+莊對(0110)
                    rData = 2;
                    rPair = 1;
                    break;
                case "A":
                    // A=閒+閒對(1010)
                    rData = 2;
                    rPair = 2;
                    break;
                case "E":
                    // E=閒+莊對+閒對(1110)
                    rData = 2;
                    rPair = 3;
                    break;
                case "7":
                    // 7=和+莊對(0111)
                    rData = 3;
                    rPair = 1;
                    break;
                case "B":
                    // B=和+閒對(1011)
                    rData = 3;
                    rPair = 2;
                    break;
                case "F":
                    // F=和+莊對+閒對(1111)
                    rData = 3;
                    rPair = 3;
                    break;
            }

            if (rawRoad[i]) {
                // 指定珠盤路位置存在
                rawRoad[i].x = xPos;
                rawRoad[i].y = yPos;
                rawRoad[i].data = rData;
                rawRoad[i].pair = rPair;
                rawRoad[i].isQueryRoad = false;
            } else {
                rawRoad[i] = {
                    x: xPos,
                    y: yPos,
                    data: rData,
                    pair: rPair,
                    RoundNumber: rawResultData[i].RoundNumber,
                    isQueryRoad: false,
                    dataImg: null,
                    ppImg: null,
                    bpImg: null
                };

                valueChange = true;
            }

            if (xPos > xPosMax) {
                xPosMax = xPos;
            }

            yPos++;
            if (yPos >= 6) {
                xPos++;
                yPos = 0;
            }
        }

        arrayMax = rawResultData.length - 1;

        // 產生問路資料
        queryRoad.IfBanker = getNextRawRoad(xPos, yPos, 1)
        queryRoad.IfPlayer = getNextRawRoad(xPos, yPos, 2)

        // 0=無問路/1=庄問路/2=閒問路
        if (queryRoadType == 1) {
            arrayMax++;

            if (rawRoad[arrayMax]) {
                rawRoad[arrayMax].data = queryRoad.IfBanker.data;
                rawRoad[arrayMax].x = queryRoad.IfBanker.x;
                rawRoad[arrayMax].y = queryRoad.IfBanker.y;
            } else {
                rawRoad[arrayMax] = queryRoad.IfBanker;
            }

            if (rawRoad[arrayMax].x > xPosMax) {
                xPosMax = rawRoad[arrayMax].x;
            }
        } else if (queryRoadType == 2) {
            arrayMax++;

            if (rawRoad[arrayMax]) {
                rawRoad[arrayMax].data = queryRoad.IfPlayer.data;
                rawRoad[arrayMax].x = queryRoad.IfPlayer.x;
                rawRoad[arrayMax].y = queryRoad.IfPlayer.y;
            } else {
                rawRoad[arrayMax] = queryRoad.IfPlayer;
            }

            if (rawRoad[arrayMax].x > xPosMax) {
                xPosMax = rawRoad[arrayMax].x;
            }
        }

        // 問路結束

        // 移除過多的 road (可能經過修改)
        if ((rawRoad.length - 1) > arrayMax) {
            // 移除綁在畫面上的物件
            for (var i = arrayMax + 1; i < rawRoad.length; i++) {
                road = rawRoad[i];

                if (road.dataImg != null) {
                    if (road.dataImg.hasAttribute("BlinkId") == true) {
                        clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                        road.dataImg.removeAttribute("BlinkId");
                    }

                    try {
                        oRoadMap.removeChild(road.dataImg);
                    }
                    catch (e) {
                    }
                }

                if (road.ppImg != null) {
                    try {
                        oRoadMap.removeChild(road.ppImg);
                    }
                    catch (e) {
                    }
                }

                if (road.bpImg != null) {
                    try {
                        oRoadMap.removeChild(road.bpImg);
                    }
                    catch (e) {
                    }
                }
            }

            rawRoad.splice(rawResultData.length, rawRoad.length - rawResultData.length)
        }

        if ((xPosMax - (rawRoadCfg.colMax - 1)) >= 0) {
            xDisplay = (xPosMax - (rawRoadCfg.colMax - 1));
        } else {
            xDisplay = 0;
        }

        // 珠盤路最大 x:12
        // 處理珠盤路貼圖
        if ((rawRoadCfg.enableDisplay == true) && (oRoadMap != null)) {
            for (var i = 0; i < rawRoad.length; i++) {
                var baseX;
                var baseY;

                road = rawRoad[i];

                if (road.x >= xDisplay) {
                    baseX = roadPosLeft + ((road.x - xDisplay) * rawRoadCfg.width);
                    baseY = roadPosTop + (road.y * rawRoadCfg.height);

                    // 處理主要資料
                    if (road.dataImg == null) {
                        road.dataImg = document.createElement("img");
                        road.dataImg.style.display = "none";
                        road.dataImg.style.position = "absolute";
                        road.dataImg.style.width = (rawRoadCfg.width - 1) + "px";
                        road.dataImg.style.height = (rawRoadCfg.height - 1) + "px";
                        road.dataImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 珠盤路圖像點下去處理函式
                            if (rawRoadCfg.eventHandler != null) {
                                road.dataImg.onclick = function () { raiseEvent(this, "click"); }
                                road.dataImg.onmouseover = function () { raiseEvent(this, "mouseover"); }
                                road.dataImg.onmouseout = function () { raiseEvent(this, "mouseout"); }
                            }
                        }

                        oRoadMap.appendChild(road.dataImg);
                    }

                    switch (rawRoad[i].data) {
                        case 1:
                            road.dataImg.src = rawRoadCfg.bImg;
                            road.dataImg.style.display = "inline";
                            break;
                        case 2:
                            road.dataImg.src = rawRoadCfg.pImg;
                            road.dataImg.style.display = "inline";
                            break;
                        case 3:
                            road.dataImg.src = rawRoadCfg.tImg;
                            road.dataImg.style.display = "inline";
                            break;
                        default:
                            road.dataImg.style.display = "none";
                    }

                    // 處理對子
                    if (road.ppImg == null) {
                        road.ppImg = document.createElement("img");
                        road.ppImg.style.display = "none";
                        road.ppImg.style.position = "absolute";
                        road.ppImg.style.width = (rawRoadCfg.width - 1) + "px";
                        road.ppImg.style.height = (rawRoadCfg.height - 1) + "px";
                        road.ppImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 珠盤路圖像點下去處理函式
                            if (rawRoadCfg.eventHandler != null) {
                                road.ppImg.onclick = function () { raiseEvent(this, "click"); }
                                road.ppImg.onmouseover = function () { raiseEvent(this, "mouseover"); }
                                road.ppImg.onmouseout = function () { raiseEvent(this, "mouseout"); }
                            }
                        }

                        oRoadMap.appendChild(road.ppImg);
                    }

                    if (road.bpImg == null) {
                        road.bpImg = document.createElement("img");
                        road.bpImg.style.display = "none";
                        road.bpImg.style.position = "absolute";
                        road.bpImg.style.width = (rawRoadCfg.width - 1) + "px";
                        road.bpImg.style.height = (rawRoadCfg.height - 1) + "px";
                        road.bpImg.setAttribute("RoundNumber", road.RoundNumber);

                        if (road.isQueryRoad == false) {
                            // 珠盤路圖像點下去處理函式
                            if (rawRoadCfg.eventHandler != null) {
                                road.bpImg.onclick = function () { raiseEvent(this, "click"); }
                                road.bpImg.onmouseover = function () { raiseEvent(this, "mouseover"); }
                                road.bpImg.onmouseout = function () { raiseEvent(this, "mouseout"); }
                            }
                        }

                        oRoadMap.appendChild(road.bpImg);
                    }


                    switch (road.pair) {
                        case 1:
                            // 庄對
                            road.bpImg.src = rawRoadCfg.bpImg;
                            road.bpImg.style.display = "inline";
                            road.ppImg.style.display = "none";
                            break;
                        case 2:
                            // 閒對
                            road.ppImg.src = rawRoadCfg.ppImg;
                            road.ppImg.style.display = "inline";
                            road.bpImg.style.display = "none";
                            break;
                        case 3:
                            // 庄閒對
                            road.bpImg.src = rawRoadCfg.bpImg;
                            road.bpImg.style.display = "inline";
                            road.ppImg.src = rawRoadCfg.ppImg;
                            road.ppImg.style.display = "inline";
                            break;
                        default:
                            road.bpImg.style.display = "none";
                            road.ppImg.style.display = "none";
                    }

                    if (road.isQueryRoad == true || road.RoundNumber == blinkRoundNumber) {
                        if (road.dataImg.hasAttribute("BlinkId") == false) {
                            var blinkId;

                            blinkId = setBlink(road.dataImg);
                            road.dataImg.setAttribute("BlinkId", blinkId);
                        }
                    } else {
                        if (road.dataImg.hasAttribute("BlinkId") == true) {
                            clearInterval(parseInt(road.dataImg.getAttribute("BlinkId")));
                            road.dataImg.removeAttribute("BlinkId");

                            clearBlink(road.dataImg);
                        }
                    }

                    if (road.dataImg != null) {
                        road.dataImg.style.left = baseX + "px";
                        road.dataImg.style.top = baseY + "px";
                    }

                    if (road.ppImg != null) {
                        road.ppImg.style.left = baseX + "px";
                        road.ppImg.style.top = baseY + "px";
                    }

                    if (road.bpImg != null) {
                        road.bpImg.style.left = baseX + "px";
                        road.bpImg.style.top = baseY + "px";
                    }
                } else {
                    if (road.dataImg != null && road.dataImg.style.display == "inline")
                        road.dataImg.style.display = "none";

                    if (road.bpImg != null && road.bpImg.style.display == "inline")
                        road.bpImg.style.display = "none";

                    if (road.ppImg != null && road.ppImg.style.display == "inline")
                        road.ppImg.style.display = "none";
                }
            }
        }
    }

    function updateCounting() {
        var countingResult = {
            banker: 0,
            player: 0,
            tie: 0,
            bankerPair: 0,
            playerPair: 0
        }

        for (var i = 0; i < rawResultData.length; i++) {
            switch (rawResultData[i].LastResult) {
                case "1":
                    // 1=莊(0001)
                    countingResult.banker++;
                    break;
                case "2":
                    // 2=閒(0010)
                    countingResult.player++;
                    break;
                case "3":
                    // 3=和(0011)
                    countingResult.tie++;
                    break;
                case "5":
                    // 5=莊+莊對(0101)
                    countingResult.banker++;
                    countingResult.bankerPair++;
                    break;
                case "9":
                    // 9=莊+閒對(1001)
                    countingResult.banker++;
                    countingResult.playerPair++;
                    break;
                case "D":
                    // D=莊+莊對+閒對(1101)
                    countingResult.banker++;
                    countingResult.bankerPair++;
                    countingResult.playerPair++;
                    break;
                case "6":
                    // 6=閒+莊對(0110)
                    countingResult.player++;
                    countingResult.bankerPair++;
                    break;
                case "A":
                    // A=閒+閒對(1010)
                    countingResult.player++;
                    countingResult.playerPair++;
                    break;
                case "E":
                    // E=閒+莊對+閒對(1110)
                    countingResult.player++;
                    countingResult.bankerPair++;
                    countingResult.playerPair++;
                    break;
                case "7":
                    // 7=和+莊對(0111)
                    countingResult.tie++;
                    countingResult.bankerPair++;
                    break;
                case "B":
                    // B=和+閒對(1011)
                    countingResult.tie++;
                    countingResult.playerPair++;
                    break;
                case "F":
                    // F=和+莊對+閒對(1111)
                    countingResult.tie++;
                    countingResult.bankerPair++;
                    countingResult.playerPair++;
                    break;
            }
        }

        return countingResult;
    }

    function clearBlink(o) {
        if (o != null) {
            o.style.filter = "alpha(opacity=100)";//"filter: alpha(opacity=70); -moz-opacity: 0.7; opacity: 0.7; display: inline;";
            o.style.opacity = 1.0;
        }
    }

    function setBlink(o) {
        var op = 100;
        var opMode = 0;

        return setInterval(function () {
            o.style.filter = "alpha(opacity=" + op + ")";//"filter: alpha(opacity=70); -moz-opacity: 0.7; opacity: 0.7; display: inline;";
            o.style.opacity = (op / 100);

            if (opMode == 0) {
                op -= 10;
                if (op <= 0) {
                    op = 0;
                    opMode = 1;
                }
            } else {
                op += 10;
                if (op >= 100) {
                    op = 100;
                    opMode = 0;
                }
            }
        }, 50);
    }
}